...
def lamb_step(
    ...
    Seef :class:`~torch.optim.AdamW` for details.
    """
...
class LearningRateDecayCallback(pl.Callback):
    def __init__(self, learning_rate, warmup_steps, num_steps):
        super().__init__()
    ...
    def on_train_batch_end(
        ...
        if self.curr_step < self.num_warmup_steps:
            lr_mult = float(self.curr_step) / float(max(1, self.num_warmup_steps))
        else:
            lr_mult = max(
                0.0, float(self.num_training_steps - self.curr_step
...
if __name__ == "__main__":
    ...
    num_steps = args.max_epochs * (len(dm.train_dataset) // (args.batch_size * args.gpus))

    lr_decay = LearningRateDecayCallback(
        learning_rate=args.learning_rate,
        warmup_steps=num_steps * 0.25,
        num_steps=num_steps,
    )
